...
import { questionObj, treatmentType } from "@redux/slices/assessment";
...
import { dermatologyQuestions } from "../../libs/dermatologyQuestions.js";
...
export interface DermatologyContainerProps {
    assessmentId: string;
...
const DermatologyContainer = (props: DermatologyContainerProps) => {
    ...
    const {
        dermatologyData,
        setDermatologyData,
    }: LayoutContextModel = useContext(LayoutContext);
    ...
    useEffect(() => {
        ...
        if (current === 1) {
            ...
            if (current === 1 && currentQuestionfromApi) {
                ...
                setCurrentQuestionObj((d: any) => {
                    const currentQuestionIndex = dermatologyQuestions.findIndex(
                        (question: any) => question.qId === activeQuestionId,
                    );
                    const nextQuestion =
                        dermatologyQuestions[currentQuestionIndex];
                    return { ...nextQuestion };
                });
            }
            if (activeQuestionId === "QUE_6") {
                const updatedImgQue =
                    assessMentDetails?.questions?.length > 0
                        ? assessMentDetails?.questions.find(
                              (item: any) => item?.qId === "QUE_6",
                          )
                        : {};
                const updatedImgObj =
                    updatedImgQue && updatedImgQue?.answers?.length > 0
                        ? updatedImgQue?.answers?.map(
            ...
            if (currentQuestionObj?.qId === "QUE_18") {
                reset({
            ...
            if (current === 1) {
                ...
                if (responseFromNext) {
                    ...
                    if (currentQuestionIndex === 17) {
                        ...
                        const selectedTriggers: any = responseFromNext?.data?.answers?.reduce(
                            (triggers: any, selectedAnswer: any) => {
                                const selectedOption = responseFromNext?.data?.options.find(
                                    (option: any) =>
                                        option.key === selectedAnswer,
                                );
                                if (selectedOption) {
                                    triggers.push(selectedOption.trigger);
                                }
                                return triggers;
                            },
                            [],
                        ...
                        const data = dermatologyQuestions.filter(
                            (question: any) =>
                                selectedTriggers.some((keyword: any) =>
                                    question.triggerName.includes(keyword),
                                ),
                        );

                        setDermatologyData(data);
    ...
    const handleOnPrevButton = () => {
        if (current === 1) {
            ...
            if (currentQuestionIndex === 0) {
                ...
                const currentQuestion = dermatologyData?.findIndex(
                    (question: any) => question.qId === currentQuestionObj.qId,
                );
                const prevQuestion = dermatologyData[currentQuestion - 1];
                if (prevQuestion) {
        ...
        if (current === 2) {
            ...
            if (currentQuestionIndex === 0) {
                setCurrent(2);
                router.query.activeQuestionId = "QUE_18";
                router.push(router);
            } else {
                const prevQuestion =
                    MedicalHistoryQuestions[currentQuestionIndex - 1];
        ...
        if (current === 3) {
            ...
            if (activeQuestionId === "treatment-frequency") {
                if (
                    assessMentDetails?.treatmentOption?.treatment?.supplementIds
                        ?.length === 0
                ) {
                    router.query.activeQuestionId = "treatment-options";
                    router.push(router);
    ...
    useEffect(() => {
        ...
        if (
            dermatologyQuestions.find((q: any) => q?.qId === activeQuestionId)
        ) {
            setCurrent(1);
        }
        if (
    ...
    const handleDisabled = () => {
        ...
        if (current === 1) {
            ...
            if (currentQuestionObj?.qId === "QUE_6") {
            ...
            } else if (currentQuestionObj?.qId === "QUE_7") {
                if (
                    currentQuestionObj?.answers?.length ===
                    assessMentDetails?.questions[0]?.answers?.length
                ) {
                    return false;
                } else {
                    return true;
                ...
                ["QUE_2", "QUE_3", "QUE_5", "QUE_11", "QUE_13"].includes(
                    activeQuestionId,
                )
            ) {
        ...
        if (current === 3) {
            if (activeQuestionId === "treatment-options") {
                if (selectedProduct && selectedProduct?.productId?.length > 0) {
                    return false;
                } else {
                    return true;
                }
    ...
    const getTitle = (currentQuestionObj: any) => {
        if (currentQuestionObj?.flagTitle) {
            ...
            currentQuestionObj?.options?.some(
                (item: any) => item?.label === "Less than 1 week",
            )
        ) {
            return "Acknowledgment Required";
        } else if (
            currentQuestionObj?.options?.some(
                (item: any) => item?.label === "2 years +",
            )
        ) {
            return "Acknowledgment Required";
        } else {
    ...
    const getDescription = (currentQuestionObj: any) => {
        ...
        } else if (assessmentFlag?.answerKeys === "QUE_14_ANS_1") {
            return "It is recommended that the scar treatment be applied only to intact or unbroken skin (if post-operative, after sutures have been removed).";
        } else if (assessmentFlag?.answerKeys === "QUE_14_ANS_4") {
            return "The effectiveness of our scar treatments may be reduced with scars that have been present for more than 2 years";
        } else {
            return "We apologize for the inconvenience, but we cannot proceed with your assessment at this time. Your responses have raised concerns, and we recommend consulting a primary healthcare provider in person for a thorough evaluation. Once cleared, you are welcome to return to seek treatment.";
        }
    };

    const ResponseData = assessMentDetails;
    // for QUE_7 to render option on response of QUE_1
    ...
    const triggeredOptionsArray = getTriggeredOptions(ResponseData);
...
DermatologyContainer.getInitialProps = wrapper.getInitialPageProps(
...
DermatologyContainer.Layout = BlankLayoutComponent;
